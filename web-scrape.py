...
for year in years:  # Iterate through years
    ...
    dataframes.append(mvp)  # Append DataFrame to list
mvps = pd.concat(dataframes)  # Concatenate DataFrames to one frame
mvps.to_csv('mvps.csv')  # Save DataFrame to CSV file
"""
""" This only runs once, to avoid long run times, creating the players.csv file
for year in years:  # Iterate through years
...
players = pd.concat(dataframes)  # Concatenate DataFrames to one frame
...
for year in years:  # Iterate through years
    ...
    dataframes.append(team)  # Append DataFrame to list
    soup = BeautifulSoup(page, 'html.parser')  # Parse webpage
    table = soup.find('tr', class_='thead').decompose()  # Find redundant row and remove
    ...
    dataframes.append(team)  # Append DataFrame to list
teams = pd.concat(dataframes)  # Concatenate DataFrames to one frame
